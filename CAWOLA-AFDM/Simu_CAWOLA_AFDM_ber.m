% Simulation of the proposed CAWOLA-AFDM transceiver
...
rollOffWin_set = [0: 0.05: 0.3];    % roll-off factor for receiving windowing
snr_set = [20: 5: 55];             % received snr (db)
numPath_set = [3, 10];
ell_max_set = [1, 10, 100];
k_max_set = [1, 3, 6];
winRx_set = [0, 1, ...  % wola-rc, woloa-cheb90
             2, 3];   % [cawola-rc-trueChan, cawola-rc-estChan]

params_set = [];
for i_sinr = 0: length(snr_set)-1
    for i_rollOff = 0: length(rollOffWin_set)-1
        for i_num = 0: length(numPath_set)-1
            for i_ell = 0: length(ell_max_set)-1
                for i_k = 0: length(k_max_set)-1
                    for i_win = 0: length(winRx_set)-1
                        params_set = [ params_set, [snr_set(i_sinr+1); ...
                                                    rollOffWin_set(i_rollOff+1); ...
                                                    numPath_set(i_num+1); ...
                                                    ell_max_set(i_ell+1); ...
                                                    k_max_set(i_k+1); ...
                                                    winRx_set(i_win+1)] ];
...
ber_all_rx = zeros(1, numParams);
nmse_all_rx = zeros(1, numParams);
numTest_all_rx = zeros(1, numParams);
...
for indxParams = 0: numParams-1
    ...
    snrDb = params_vec(1);
    rollOffWin = params_vec(2);
    numPath = params_vec(3);
    ell_max = params_vec(4);
    k_max = params_vec(5);
    winRx_type = params_vec(6);
    ...
    for indxTest = 0: numTest-1
        ...
        rollOffWin = lenCp_ext / numRe;   % roll-off factor modification
        winTx = rectwin(numRe).';   % transmit shaping window
        winRx_temp = raisedCosWin(rollOffWin, numRe);   % receive shaping window
        if winRx_type == 0
            winRx = winRx_temp;   % receive shaping window
        elseif winRx_type == 1
            winRx = chebwin(numRe+lenCp_ext, 90).';
        elseif winRx_type == 2
            winRx = chanAwareWolaWinRx(winRx_temp, numRe, lenCp_ext, chirpRate, ...
                winOrder, numPath, abs(tapGainPath).^2, tapDopplerPath);
        elseif winRx_type == 3
            ...
            [yAll, yNoiseAll] = afdmTransceiver(xAll, a_wgn, ...
                numRe, lenCp, lenCp_ext, winTx, winRx_temp, ...
    ...
    nmse_all_rx(indxParams+1) = nmseEst / (indxTest+1);
    ber_all_rx(indxParams+1) = berEst / (indxTest+1);
    numTest_all_rx(indxParams+1) = indxTest+1;
...
save('data_ber_all_rx.mat', 'ber_all_rx');
save('data_nmse_all_rx.mat', 'nmse_all_rx');
save('data_numTest_all_rx.mat', 'numTest_all_rx');
