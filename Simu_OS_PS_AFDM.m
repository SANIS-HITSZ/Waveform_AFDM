    ...
    for indxWin = 0: (length(rollOffWin_set)+2)-1
        ...
        if indxWin < length(rollOffWin_set)
            hData = inOutAfdm_rxWin(numSc, indxSc_schd, lenCp_ext, ...
                chirpRate, prechirpRate, rollOffTime, lenPulseTime, winRx, ...
                numPath, tapGainPath, tapDelayPath, tapDopplerPath);
            % condition number
            [~, S, ~] = svd(hData'*hData);
            S = sqrt(abs(diag(S)));
            ...
            hData = inOutAfdm_txWin(numSc, indxSc_schd, ...
                chirpRate, prechirpRate, rollOffTime, lenPulseTime, winTx, ...
                numPath, tapGainPath, tapDelayPath, tapDopplerPath);
            % condition number
            [~, S, ~] = svd(hData'*hData);
            S = sqrt(abs(diag(S)));
        ...
        numDoppler = abs(2 * numSc * chirpRate);
        ...
        hMat_est = chanMat_est(yPilot, xPilot, indxPilot, ...
            numSc, indxSc_schd, chirpRate, prechirpRate, labelDelay, labelDoppler);
...
function hData = inOutAfdm_rxWin(numSc, indxSc_schd, lenCp_ext, ...
    ...
    %             m_shift = m_prime - (2*numSc*chirpRate*ell_prime - tapDoppler);
    %             winRx_phase = winRx ...
    ...
    %                 * exp(1i * 2*pi * prechirpRate*(m_prime^2-[0:numSc-1].^2));
    ...
    dePrechirpPhase = exp(- 1i * 2*pi * prechirpRate*[0:numSc-1].^2).';
    for indxSc = 0: numSc_schd-1
    ...
    %                     * exp(1i * 2*pi * chirpRate*ell_prime^2) ...
    %                     * exp(- 1i * 2*pi/numSc * m_prime*ell_prime)...
    %                     * exp(1i * 2*pi * prechirpRate*(m_prime^2-m.^2));
...
function hMat_est = chanMat_est(yPilot, xPilot, indxPilot, ...
    ...
    for indxDoppler = 0: numDoppler-1
        for indxDelay = 0: numDelay-1
            ...
            indxX_shift = 2*numSc*chirpRate*valDelay - valDoppler;
            indxY = mod(indxPilot-indxX_shift, numSc);
            phaseCompTerm = exp(1i * 2*pi * prechirpRate * (indxPilot^2 - indxY.^2)) ...
                * exp(1i * 2*pi * chirpRate*valDelay^2) ...
    ...
    for indxDelay = 0: numDelay-1
        for indxDoppler = 0: numDoppler-1
            ...
            indxX_shift = 2*numSc*chirpRate*tapDelay - tapDoppler;
            chanCoef = tapGain ...
                    * exp(1i * 2*pi/numSc * tapDelay*tapDoppler) ...
                    * exp(- 1i * 2*pi * chirpRate*tapDelay^2) ...
                    * exp(- 1i * 2*pi * prechirpRate*indxX_shift^2);
            for indxSc = 0: numSc_schd-1
                indxX = indxSc_schd(indxSc+1);
                indxY = mod(indxX - indxX_shift, numSc);
                phaseTerm = exp(1i * 2*pi/numSc * (2*numSc*prechirpRate*indxX_shift-tapDelay)*indxY);
